...
Select
   ...
   Consulta.DEMANDAS_CF,
   Consulta.CONSUMO_MENSAL_CF,
   Consulta.DEMANDAS_JEFAZ,
   Consulta.CONSUMO_MENSAL_JEFAZ,
   Consulta.ESTOQUE,
   Consulta.AUTONOMIA,
                        ...
                        SELECT
                           ...
                           CASE
                              ...
                              THEN
                                 'MARCA'
                           END
                           ORIGEM, I.MAR_DESCRICAO, I.PRO_IMPORTADO, I.PRO_OUT_DEM, I.PRO_ID, PRO_CODIGO, FCN_NOME_PRODUTO(I.PRO_ID) NOME, PRO_SIAFISICO, P.PRO_CATMAT as CATMAT, NVL(SUM(DEMANDAS), 0) DEMANDAS, NVL(SUM(DECODE(PED_TIPO, 1, DEMANDAS)), 0) DEMANDAS_AJ, NVL(SUM(CONSUMOMENSAL), 0) CONSUMO_MENSAL, NVL(SUM(DECODE(PED_TIPO, 1, CONSUMOMENSAL)), 0) CONSUMO_MENSAL_AJ, NVL(SUM(DECODE(PED_TIPO, (SELECT MIN(tpd_id) FROM tipo_demanda WHERE UPPER(tpd_descricao) LIKE 'COMISS%FARMACOLOG%'), DEMANDAS)), 0) DEMANDAS_CF, NVL(SUM(DECODE(PED_TIPO, (SELECT MIN(tpd_id) FROM tipo_demanda WHERE UPPER(tpd_descricao) LIKE 'COMISS%FARMACOLOG%'), CONSUMOMENSAL)), 0) CONSUMO_MENSAL_CF, NVL(SUM(DECODE(PED_TIPO, (SELECT MIN(tpd_id) FROM tipo_demanda WHERE UPPER(tpd_descricao) LIKE 'JEFAZ%'), DEMANDAS)), 0) DEMANDAS_JEFAZ, NVL(SUM(DECODE(PED_TIPO, (SELECT MIN(tpd_id) FROM tipo_demanda WHERE UPPER(tpd_descricao) LIKE 'JEFAZ%'), CONSUMOMENSAL)), 0) CONSUMO_MENSAL_JEFAZ, ESTOQUE - ESTOQUE_VENCIDO AS ESTOQUE,
                           CASE
                              WHEN
