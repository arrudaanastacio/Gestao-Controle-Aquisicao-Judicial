...
router.get('/requisicoes', (req, res) => {
  ...
  const veTodas = ehAdmin || usuarioVeTodas(req.usuario); // permissão da aba "Todas"
  ...
  res.json({
    ...
    caixas: { ehAdmin, veTodas, visiveis: caixasVisiveis, contagens, totalPermitido },
  });
});
...
router.get('/requisicoes/itens', (req, res) => {
  ...
  const ehAdmin = permitidas === null;
  // "Todas" é uma PERMISSÃO de aba: quem tem vê a aba "Todas"; quem não tem só
  // vê as abas das caixas que possui. Não altera o conteúdo (segue a união das
  // caixas permitidas).
  const veTodas = ehAdmin || usuarioVeTodas(req.usuario);
