...
router.get('/reposicao', (req, res) => {
  ...
  res.json({
    ...
    dataImportacaoEstoque: (ultimaData && db.prepare("SELECT datetime(criado_em,'localtime') q FROM estoque_importacoes WHERE data_referencia=? LIMIT 1").get(ultimaData.data_referencia)?.q) || null,
    dataReferenciaOperador: dataOperador,
    dataImportacaoOperador: (dataOperador && db.prepare("SELECT datetime(criado_em,'localtime') q FROM estoque_od_importacoes WHERE data_referencia=? LIMIT 1").get(dataOperador)?.q) || null,
