...
|---|----------|----------------------|------|--------|
| 34 | **Cache-buster automático do frontend (fim do Ctrl+F5).** O `index.html` tinha `app.js?v=66` travado, então o navegador guardava o JS antigo e não rebuscava — causa de "não mudou nada" após deploys. Agora o servidor serve o index (rotas `/` e `/index.html`, com `Cache-Control: no-cache`) trocando a versão de `app.js` e `estilo.css` pela **data de modificação do arquivo**. Toda mudança no frontend passa a ser buscada com um **F5 normal**. **Requer reiniciar o servidor** (mudança no `server.js`). | (a registrar) | 21/07/2026 | ✅ Verificado: index serve versão por mtime |
| 33 | **Visual ERP — cartões de KPI ricos também nas telas OD.** Relatório de Compras OD (Total, Em andamento, Finalizadas, Entrega parcial) e Aquisição em Andamento OD (Total em andamento, Empenhadas, Entrega parcial, Planejamento) trocam os cartões simples por status pelos **cartões `.cartao-kpi`** (ícone + número + descrição), no mesmo padrão das telas TP. Agora as 4 telas de relatório usam o mesmo estilo. Ver [[direcao-visual-erp]]. | 3e4807c | 21/07/2026 | ✅ Lógica conferida |
| 32 | **Visual ERP — alinhamento com o mockup.** Comparado o mockup com o entregue e corrigido: (1) cartões de KPI dos relatórios agora têm **ícone + número + linha descritiva** (`.cartao-kpi`, como no mockup) em vez de só número+rótulo; (2) botão **"Atualizar agora" verde** (primário) em vez de contornado; (3) **TIPO** (JS/AS/JM/ASM) vira **etiqueta** (`.tag-tipo`) nas 4 tabelas de relatório; (4) cores de status (âmbar/vermelho/verde) clareadas no tema escuro para legibilidade. Ver [[direcao-visual-erp]]. | bc7dfd1 | 21/07/2026 | ✅ Conferido no DOM (claro e escuro) |
| 31 | **Visual ERP — tema claro/escuro (identidade do mockup).** Botão ☀️/🌙 na topbar; cada pessoa escolhe claro ou escuro (guardado no navegador). O escuro reproduz a identidade do mockup (fundo `#0f1d19`, cartões escuros, números verde-claro). Feito via variáveis: cores cravadas das tabelas viraram tokens (`--zebra`, `--realce-tabela`, `--linha-tabela`, `--topbar-fundo`). Também deixa as etiquetas de status em formato **pílula** (raio 20px). **Nota:** 1ª versão cobre as superfícies principais (fundo, cartões, tabelas, topbar, chips, botões); alguns componentes específicos (grade de Distribuição, alguns modais/badges) podem ainda ter manchas claras no escuro — refino conforme aparecerem. Ver [[direcao-visual-erp]]. | 1b2c813 | 21/07/2026 | ✅ Cores conferidas nos 2 temas |
| 30 | **Visual ERP — Fase 4 (KPIs), cobertura das telas de relatório.** Cartões de indicadores na **Tabela Análise TP** (Total, Em andamento, Finalizadas e **Atrasadas** — este fica vermelho se houver), calculados do `/solicitacoes/resumo` (totais exatos, não paginados). As telas OD (Relatório de Compras OD e Aquisição em Andamento OD) já tinham cartões de resumo. Agora as 4 telas de relatório mostram KPIs. Continuação da direção [[direcao-visual-erp]]. | 8a1a0e1 | 21/07/2026 | ⏳ A conferir em homologação |
| 29 | **Visual ERP — Fase 4 (KPIs reais nos relatórios), início.** Cartões de indicadores no topo do **Relatório de Compras TP** (Solicitações no filtro atual, Em andamento, Finalizadas, Itens distintos), calculados no navegador a partir das linhas já carregadas — refletem o filtro (ano/status/busca) da tela, sem endpoint novo. Padrão pronto para replicar nas demais telas de relatório. Quarta etapa da direção [[direcao-visual-erp]]. | 9a1e6a2 | 21/07/2026 | ⏳ A conferir em homologação |
