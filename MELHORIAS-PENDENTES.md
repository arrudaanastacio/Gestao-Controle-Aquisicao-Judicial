...
|---|----------|----------------------|------|--------|
| 24 | **Botão "Atualizar agora" (só admin)** nas telas Relatório de Compras TP, Tabela Análise TP, Relatório de Compras OD e Aquisição em Andamento OD. Relê o arquivo da pasta de rede e reimporta na hora, sem esperar os horários agendados (12h/19h) — importante para ação judicial, onde os dados precisam ir o mais atualizados possível. Como TP e OD vêm cada um de UM arquivo, um clique atualiza as duas telas da mesma fonte. Endpoints admin-only (`/importar-solicitacoes/atualizar-agora` e `/solicitacoes-od/atualizar-agora`) que reaproveitam o "refazer o mês" dos vigias. | (a registrar) | 21/07/2026 | ⏳ A conferir em homologação |
