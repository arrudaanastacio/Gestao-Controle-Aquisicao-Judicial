...
|---|----------|----------------------|------|--------|
| 18 | Serviço do Windows — sistema sobe sozinho ao ligar o PC, reinicia sozinho se travar, roda sem janela aberta. Instalação por duplo-clique (`9 - instalar/desinstalar-servico-windows.bat`). Cria serviços separados para produção (`ComprasJudiciais`) e homologação (`ComprasJudiciasHomolog`). **Falta: Rafael testar a instalação real na homologação (exige Administrador).** | _(a preencher)_ | 20/07/2026 | Em teste |
