...
|---|----------|----------------------|------|--------|
| 29 | **Visual ERP — Fase 4 (KPIs reais nos relatórios), início.** Cartões de indicadores no topo do **Relatório de Compras TP** (Solicitações no filtro atual, Em andamento, Finalizadas, Itens distintos), calculados no navegador a partir das linhas já carregadas — refletem o filtro (ano/status/busca) da tela, sem endpoint novo. Padrão pronto para replicar nas demais telas de relatório. Quarta etapa da direção [[direcao-visual-erp]]. | (a registrar) | 21/07/2026 | ⏳ A conferir em homologação (lógica testada; cálculo confere) |
