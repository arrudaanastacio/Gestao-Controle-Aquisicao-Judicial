...
| 8 | Movimentação de Entrada Estoque — tela no menu Tenente Pena puxando via Oracle a query enviada pelo Rafael (só o bloco Entrada, só Unidade Tenente Pena), janela móvel de 12 meses calculada no próprio SQL, ordenada da mais nova para a mais antiga, filtro por tipo de movimentação, encadeada no agendador diário após Autores. **Ainda falta validar a query contra o Oracle real** (só testei com dados simulados neste ambiente) | `f2c1803`, `80cfffa` | 13/07/2026 | Pendente — precisa validar Oracle |
